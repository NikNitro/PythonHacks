...
__author__ = 'Lenny Domnitser <http://domnit.org/>, Adrián M. Lloret https://github.com/NikNitro/>'
__version__ = '0.3'
# I (Adrián M. Lloret) have only adapted the code for using it in Python 3.x
...
def encode(filepath, message):
    ...
    stegimg = stepic3.encode(img, message)
    stegimg.save(filepath+'_crypted.png', 'PNG')

# Funcion que descodifica el mensaje de la imagen
def decode(filepath):
    img = Image.open(filepath)
    message = stepic3.decode(img)
    return message
"""